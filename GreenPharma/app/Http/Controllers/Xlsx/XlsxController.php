	...
	private function _unzip( $filename, $is_data = false ) {
		...
		if ( $is_data ) {
			...
			if ( ! is_readable( $filename ) ) {
				$this->error( 1, 'Nenhum Arquivo Selecionado ' . $filename );
		...
		*/
		...
		$aEL = count( $aE );
		if ( $aEL === 0 ) {
			$this->error( 2, 'Formato do arquivo incorreto!' );
...
//		$this->error( 'Invalid cell index ' . $cell );
	...
	public function store(){
		...
		if (isset($_FILES['file'])) {
            ...
            if ( $xlsx = $this->parse( $_FILES['file']['tmp_name'] ) ) {
                ...
                $title = array(); // Salva primeira linha
                
                foreach ( $xlsx->rows() as $k => $r ) {
                    if($k == 0){
                        for ( $i = 0; $i < $cols; $i ++ ) {
                            array_push($title,$r[$i]);
                        }
                    }else{
                        $products = array();
                        $data_products = array();
                        for ( $i = 0; $i < $cols; $i ++ ) {
                            if($i < 4){
                                array_push($products,$r[$i]);
                            }else{
                                array_push($data_products,$r[$i]);
                                array_push($data_products,$title[$i]);
                            }
                        }
                        ...
                        $id = $this->setProduct($industria,$unidade,$produto,$ean,$desc,$forn); // insere o produto
                        
                        if($id){
                            for($i = 0, $y = 1 ;$y < count($data_products); $i += 2,$y += 2){
                                if(!is_numeric($data_products[$i])){
                                    $data_products[$i] = ' ';
                                }
                                $comp = explode('/',$data_products[$y]);
                                $ano = $comp[1];
                                $mes = $comp[0];
                                if($mes < 10){
                                    $mes = '0'.$mes;
                                }
                                $date_comp = $ano."-".$mes."-01";
                                $this->setDateProduct($id,$date_comp,$data_products[$i]); // insere a data e valor
                            }
                        }else{
                ...
                return redirect()->back()->withInput()->with('success', 'Importação concluida com sucesso!');  

            } else {
                return redirect()->back()->withInput()->withErrors([$this->parseError()]);
            }
            
        }        
    }
    private function setProduct($industria,$unidade,$produto,$ean,$desc,$forn){
        
        $id = DB::table('products')->insertGetId( ['industria' => $industria, 'unidade' => $unidade,'cod' => $produto, 'ean' => $ean,'descrition' => $desc,'provider' => $forn ] ); 
        return $id;

    }
    private function getProduct($produto,$ean){
        
        $conf = DB::select("select * FROM products WHERE cod = '$produto' and ean = '$ean' ");
        if($conf){
            return false;
        }else{
            return true;
    ...
    private function setDateProduct($id,$date,$value){
        
        if($value == ' '){
            $ref = DB::table('product_dates')->insertGetId( ['fk_product' => $id, 'competence' => $date ] ); 
        }else{
            $ref = DB::table('product_dates')->insertGetId( ['fk_product' => $id, 'competence' => $date,'valor' => $value ] ); 
        }        
        return $ref;
